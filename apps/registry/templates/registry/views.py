...
def calculate_engineering_budget(data):
    """Инженерийн байгууламжийн төсөв тооцоолох"""
    ...
    if eng_type == 'heat_pit':
        length = float(data.get('length', 3.5))
        width  = float(data.get('width',  1.5))
        depth  = float(data.get('depth',  2.3))
        thick  = float(data.get('wall_thick', 0.45))
        concrete_grade = data.get('concrete_grade', 'B25')

        outer_l = length + thick * 2
        outer_w = width  + thick * 2

        # Бетоны хэмжээ
        floor_vol  = round(outer_l * outer_w * thick, 2)
        wall_vol   = round(2 * (outer_l + outer_w) * depth * thick, 2)
        total_vol  = round(floor_vol + wall_vol, 2)

        # Арматур (80 кг/м³)
        rebar_kg = round(total_vol * 80, 0)

        # Малтлага (+0.5м хажуу талд)
        exc_vol = round((outer_l + 1.0) * (outer_w + 1.0) * (depth + 0.5), 2)

        # Үнэ (UB 2026)
        concrete_price = {'B20': 260000, 'B25': 280000, 'B30': 310000}.get(concrete_grade, 280000)
        rebar_price    = 3200
        exc_price      = 25000
        formwork_price = 45000
        labor_price    = 85000

        materials.append({'name': f'Бетон {concrete_grade}', 'unit': 'м³', 'qty': total_vol,
                          'price': int(concrete_price * quality_coef),
                          'total': int(total_vol * concrete_price * quality_coef)})
        materials.append({'name': 'Арматур φ16 A400', 'unit': 'кг', 'qty': rebar_kg,
                          'price': int(rebar_price * quality_coef),
                          'total': int(rebar_kg * rebar_price * quality_coef)})
        materials.append({'name': 'Хавтан (coffering)', 'unit': 'м³', 'qty': total_vol,
                          'price': int(formwork_price * quality_coef),
                          'total': int(total_vol * formwork_price * quality_coef)})

        labor.append({'name': 'Малтлага', 'unit': 'м³', 'qty': exc_vol,
                      'price': exc_price, 'total': int(exc_vol * exc_price)})
        labor.append({'name': 'Арматурчин, бетончин ажил', 'unit': 'м³', 'qty': total_vol,
                      'price': int(labor_price * quality_coef),
                      'total': int(total_vol * labor_price * quality_coef)})
    ...
    # ── УСНЫ ХУДАГ ─────────────────────────────────────────────
        ...
        diameter = float(data.get('diameter', 1.5))
        depth    = float(data.get('depth', 3.0))
        thick    = 0.25

        vol = round(3.14 * ((diameter/2 + thick)**2 - (diameter/2)**2) * depth, 2)
        rebar_kg = round(vol * 70, 0)
        exc_vol  = round(3.14 * ((diameter/2 + thick + 0.5)**2) * (depth + 0.3), 2)
        ...
        labor.append({'name': 'Малтлага', 'unit': 'м³', 'qty': exc_vol,
                      'price': 28000, 'total': int(exc_vol * 28000)})
    ...
    # ── ДУЛААНЫ ШУГАМ ──────────────────────────────────────────
        ...
        pipe_len  = float(data.get('pipe_length', 100))
        pipe_dia  = data.get('pipe_diameter', '219мм')
        insulated = data.get('insulated', True)

        pipe_price = {'57мм': 45000, '108мм': 85000, '159мм': 120000,
                      '219мм': 180000, '273мм': 240000}.get(pipe_dia, 180000)
        ins_price  = int(pipe_price * 0.3) if insulated else 0
        trench_vol = round(pipe_len * 0.8 * 1.5, 2)
        ...
        if insulated:
            materials.append({'name': 'Дулаалга (PPU)', 'unit': 'м', 'qty': pipe_len,
        ...
        labor.append({'name': 'Шуудуу малтах', 'unit': 'м³', 'qty': trench_vol,
                      'price': 20000, 'total': int(trench_vol * 20000)})
    ...
    # ── АРИУТГАХ ТАТУУРГА ───────────────────────────────────────
        ...
        pipe_len = float(data.get('pipe_length', 50))
        manholes = int(data.get('manholes', 2))

        materials.append({'name': 'PVC хоолой D315', 'unit': 'м', 'qty': pipe_len,
                          'price': int(65000 * quality_coef),
                          'total': int(pipe_len * 65000 * quality_coef)})
        materials.append({'name': 'Шуудуу худаг', 'unit': 'ш', 'qty': manholes,
                          'price': 1500000, 'total': manholes * 1500000})
        labor.append({'name': 'Малтлага, тавих ажил', 'unit': 'м', 'qty': pipe_len,
        ...
        if length and width:
            try:
                ...
                if norm_data.get('building_type') == 'engineering':
                    result = calculate_engineering_budget(norm_data)
                else:
